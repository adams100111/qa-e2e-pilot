...
- **Global (opt-in, a SEED only):** `~/.claude/qa-e2e-pilot/roles/<project-key>.json`, where
...
2. **Seed from global (only if `--from-global`).** If `~/.claude/qa-e2e-pilot/roles/<project-key>.json`
...
5. **Save to global (only if `--global`).** After a successful confirmation, write the confirmed
   `personas[]` + authz-matrix to `~/.claude/qa-e2e-pilot/roles/<project-key>.json` (create the dir if
