...
qa-kit's bundled `constitution.sh` (`{{PLUGIN_ROOT}}/scripts/constitution.sh`) — it invents no
...
1. **Roles.** Run the engine's role flow end to end by invoking its skills (qa-kit depends on the
   ...
   `{{SKILL_REF:discovering-user-roles}}` → `{{SKILL_REF:confirming-discovered-roles}}`, which writes
...
2. **Compute the new version.** Extract just the `personas` array out of `.qa/config.json`
   ...
   ```
   bash "{{PLUGIN_ROOT}}/scripts/constitution.sh" version <personas-tmp> .qa/authz-matrix.json
   ```
...
3. **Informational diff (only if a prior constitution exists).** If `.qa/constitution.state.json`
   ...
   - Compute the new machine state to a temp file:
     `bash "{{PLUGIN_ROOT}}/scripts/constitution.sh" state <personas-tmp> <new-version> > <new-state-tmp>`.
   - Run `bash "{{PLUGIN_ROOT}}/scripts/constitution.sh" diff .qa/constitution.state.json <new-state-tmp>`.
...
4. **Write the constitution.** Two files, both stamped with the same `<personas-tmp>` +
   ...
   - `.qa/constitution.state.json` ← `bash "{{PLUGIN_ROOT}}/scripts/constitution.sh" state
   ...
   - `.qa/constitution.md` ← `bash "{{PLUGIN_ROOT}}/scripts/constitution.sh" render <personas-tmp>
     <new-version> "{{PLUGIN_ROOT}}/templates/constitution-template.md" <timestamp>` (human doc;
