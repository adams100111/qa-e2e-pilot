...
cp "$ROOT/dist/opencode/commands/"*.md "$PROJ/.opencode/command/"
...
echo "Automatic enforcement floor: --save-session -> session-preflight -> qa-verify (high-confidence). Optional live-hook hardening: see harnesses/opencode/hooks.md (verify on your build)."
