#!/usr/bin/env python3
"""Register the local adapter and delegate to SkillOpt's evaluation CLI."""
...
def _load_upstream():
    ...
    if not (source / "scripts" / "eval_only.py").is_file():
        raise SystemExit(f"SkillOpt source checkout not found: {source}")
    sys.path.insert(0, str(source))
    from scripts import eval_only as upstream
