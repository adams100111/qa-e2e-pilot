...
> **Two plugins in this repo.** `qa-e2e-pilot` (below) is the **verification engine**. `qa-kit/` is an
> optional, step-gated **process shell** built on it (constitution → spec → scenarios → analyze → run) —
> a second, separately-installable plugin that depends on the engine and leaves it unchanged. See
> [`qa-kit/README.md`](./qa-kit/README.md) and [ADR-0022](./docs/adr/0022-qa-kit-process-shell.md).
...
`qa-e2e-pilot` runs a QA pass in one shot (`/qa-run`). **qa-kit** ([`qa-kit/`](./qa-kit)) adds a
spec-kit-style **process** on top: a sequence of gated steps, each producing one human-reviewable
artifact, before the (expensive) run.

```
/qa-constitution   →  /qa-spec <target>  →  /qa-scenarios <target>  →  /qa-analyze <target>  →  /qa-run
project roles +       immutable role         scenarios + the frozen      advisory coverage        (the engine's
policy + version      snapshot (stamped +    checklist.json (the         gate (read-only)         existing run,
                      drift advisory)        enforcement contract)                                ingests the checklist)
```

It's a **second, separately-installable plugin** that declares `dependencies: ["qa-e2e-pilot"]`, reuses
...
[ADR-0022](./docs/adr/0022-qa-kit-process-shell.md).
