...
**Oracle grade**:
The strength of the expectation source backing a **suspicion**, one of `definite-dom | definite-catalog | standards | behavioral-observed | heuristic` (strongest to weakest). `definite-dom`/`definite-catalog` findings are backed by a definite **Oracle** and may become a `fail` verdict at **Confidence** `high`; `standards` may become a `fail` at `low`; `behavioral-observed` (an observed interaction-invariant violation, e.g. an overlay destroying its parent) is always a `fail` — `low` on observation alone, `high` once the shared-state cause is code-localized; `heuristic` never grounds a verdict on its own — it stays advisory unless a definite-grade finding corroborates it. An internal classification input the adjudicator computes per finding — never itself a **Verdict**, and never the same field as **Confidence** (grade decides verdict-vs-advisory and feeds the high/low split; it is not the attribute reported on the finding).
_Avoid_: confidence (a verdict attribute the grade merely informs), verdict (grade is an input to one, never a value of one).

**Overlay stack**:
The ordered set of on-screen overlays (dialog / sheet / drawer / modal) with parentage, tracked across an action sequence — a child overlay must **stack on** its parent, not replace it. The interaction/behavioral detector family drives a multistep flow and holds this expectation across the sequence; opening a child that destroys the parent (rather than stacking on it) is the canonical sheet-stack bug ADR-0019 was written to catch.
_Avoid_: conflating it with a single modal (the stack is the *set*, tracked with parentage, not one dialog in isolation); z-index alone (parentage across the sequence is the point, not paint order).

**Interaction invariant**:
One of the five behavioral rules the interaction/behavioral detector family checks against an **Overlay stack**: overlay-stack integrity, return-to-context, no-dead-end, focus-trap, no-destructive-on-open. An observed violation is a `fail @ FE` — **Confidence** `low` on observation alone, `high` once the shared `open`/route state the violation stems from is localized in code.
_Avoid_: treating it as a **Verdict** itself (it is the rule being checked; the violation it produces is the fail); treating it as a value-correctness check (that is **baking** / `verifying-computed-logic`'s domain, not presentation behavior).

**Generative critic**:
Layer 3 of the human-eye-UX engine: the multimodal model reading a screenshot + interaction trace + persona/locale together and asking what looks broken, wrong, confusing, or off. An **expectation heuristic**, not an **Oracle** — it emits `critic-<slug>` **suspicions** only, never a verdict directly; a suspicion becomes a `fail` only when a definite **Oracle** corroborates it via **adjudication**. Its coverage of the long tail is **estimated, never measured** — it is excluded from the C1 accuracy-harness's measured recall gate, since recall on unseeded, unknown bugs can't be measured.
