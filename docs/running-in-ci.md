...
- `<property name="qa.foldAnomalies" value="rule=N rule=N …"/>` — space-separated `rule=count` pairs in sorted rule order, emitted only when the fold reported at least one anomaly, and including the two record-damage rules (which are *also* named in `qa.unverifiedReason`). An absent or malformed `fold-anomalies.json` yields no property rather than invented damage. What this property changes is **visibility only** — the fold has always written these anomalies; nothing new is being detected and nothing new fails. In particular `verdict-without-started` is **not** expected on a normal run: `criterion_started` is emitted by `journal-emit.sh started`, which the orchestrator calls at each criterion's start, and `checkpoint.sh` derives the verdict's `scenarioId` with the same convention (the persona id, or `__shared__`), so the tuples match and the count stays zero. It appears when `checkpoint.sh` is driven **directly** — a script, a fixture, or a harness that skips the orchestrator's Verify-phase emission — or when a `criterion_started` arrives only *after* the verdict it belongs to.
