...
**Scope note — this is Plan A (the substrate) of a two-plan split.** It ships D-1, D-2, D-3, D-4, D-8 from the spec's ticket cut and depends on nothing unbuilt. **Plan B** (idempotency reconciliation via a *new* write-set re-bake primitive, `/qa-resume` + `.qa/runs/latest` + `plan_frozen`/`plan_amended` resume, per-harness resume tests — D-5/D-6/D-7) is a follow-up. This plan therefore **defines** the `act_intent`/`act_committed`/`plan_frozen`/`criterion_started`/`phase_entered` event *types* (so the journal schema is complete and forward-compatible) and implements+tests their **fold rules against fixtures**, but the **event *emission* wiring** (the orchestrator/`checkpoint.sh` calls that would produce start/phase/act events in a live run) and the **reconciliation/resume behaviour** that consumes them are **Plan B** (grill Q1/Q9). In Plan A the only event a live `checkpoint.sh` emits is `criterion_verdict`; the cursor/act machinery is therefore computed-and-fixture-tested here, fed for real in Plan B. Where this plan surfaces an open act, it exposes it in the fold (`openActs[]`) without acting on it.
...
- **Atomic + durable writes, degrading honestly.** Derived files are written temp-in-same-dir → (python3 `os.fsync` on the file) → `rename` → (python3 `os.fsync` on the parent dir). On a **jq-only** box (no python3) the fsync steps are skipped: the write is still POSIX-atomic (temp→rename) but **not** crash-fsync-durable, and a one-line note is appended to `fold-anomalies` (`{"rule":"fsync-unavailable"}`) so the reduced durability is recorded, never silent (for non-fold callers of `atomic_write`, e.g. `checkpoint.sh` writing `checkpoint.json`, the `FSYNC_UNAVAILABLE` fd-3 signal is dropped — the durability degrade is uniform and already recorded once per fold, so no separate channel is needed).
- **Journal append atomicity boundary (grill Q5).** A single-line `>>` append is atomically all-or-nothing only below `PIPE_BUF` (4096 B). A `criterion_verdict` with many `evidenceRefs`/a long `lastAction` can exceed that, so a crash *mid-append* can tear the line. This is the accepted durability boundary: the fold's torn-last-line rule skips exactly that in-flight event, and the only loss is the verdict actively being written at crash time — which the caller retries by re-running `checkpoint.sh`. Task 1 states the 4 KB caveat in `journal_append`'s doc comment. Do **not** add a heavier per-event temp→rename dance.
- **Full re-fold per checkpoint is accepted (grill Q4).** Each upsert appends one event then re-reads + re-folds the whole journal; `next_seq` also re-reads the file. That is O(n) per checkpoint / O(n²) per run, but *n* is bounded by `criteriaBudget` and journals are kilobytes — microseconds in practice. YAGNI: no snapshot/incremental-fold cache. Task 2/3 record the bound in a comment.
- **Scenario cursor + `cursor.json` (grill Q1/Q2).** `scenarioId == personaId` when a persona is set; a criterion with `persona:""`/absent maps to the synthetic scenario id `__shared__`. The atomic resumable unit is `(scenarioId, criterionId)`. This reconciles ADR-0012's existing `(criterion_id, persona="")` records (empty persona ⇒ `__shared__`). The fold writes the cursor/phase/counts to a **separate `cursor.json`** artifact — it does **NOT** machine-write `run-manifest.json` or `bug-log.json` (those stay agent-authored; a fold overwrite would clobber their richer hand-filled fields — grill Q2/Q3). `bug_logged` events are journaled for schema completeness but no bug-log file is folded in Plan A.
- **Resume-order parity is an invariant (grill Q7).** The fold orders `criteria[]` by **first-seen tuple position** with **last-wins verdict value**, so re-verdicting an earlier criterion keeps its original slot and `cmd_resume`'s `.criteria | last` matches legacy in-place-upsert behaviour. Task 3 adds an explicit characterization case (upsert C1, C2, re-upsert C1 → order stays `[C1,C2]`, resume returns C2).
- **Fan-out cross-child duplicate detection (grill Q8).** Child events carry a `childId`; the merge dedups on `(childId, childSeq)` so a mid-merge-crash re-run is a no-op (grill Q6). If the fold ever sees one `(scenario,criterion,persona)` tuple with verdicts from two different `childId`s (an ADR-0003 independence violation), it emits a `{"rule":"cross-child-duplicate"}` anomaly rather than silently picking a winner.
...
- Produces (sourced or shelled by later tasks + `checkpoint.sh`):
  ...
  - `journal_append <run-id> <event-json>` — validates `<event-json>` is a single JSON object with a non-empty string `event` field, stamps `seq` (current max seq in the file + 1, or 1) and `t` (`date -u +%Y-%m-%dT%H:%M:%SZ` — **the only place a wall-clock time enters the system**; fold never calls `date`), and appends exactly one line (compact, newline-terminated) via `>>`. Errors (non-zero + message) on malformed input. **Doc-comment the 4 KB atomicity caveat (grill Q5):** an append above `PIPE_BUF` (4096 B) is not guaranteed torn-free on a crash; the fold's torn-last-line rule recovers by skipping the in-flight event, which the caller retries — this is the accepted boundary, not a bug to engineer around.
...
Add to `tests/checkpoint/run.sh` (a new block after the existing cases, before the summary): assert `journal.ndjson` exists after an upsert and its last line is a `criterion_verdict` with the right `criterionId`/`verdict`/`personaId`; assert deleting `checkpoint.json` then `bash fold.sh <run>` reproduces the same `.criteria[0]` record (canonically). **Resume-order parity case (grill Q7):** upsert `C1 pass`, then `C2 pass`, then re-upsert `C1 fail` → assert `[.criteria[].criterion_id] == ["C1","C2"]` (C1 keeps its **first-seen** slot, not moved to the end) and that `checkpoint.sh --resume <run>` returns **C2** as the last criterion (matching legacy in-place-upsert behaviour) — this pins that the fold orders by first-seen position with last-wins value. Run: `bash tests/checkpoint/run.sh` → the **new** cases FAIL (journal not written yet), existing cases still `ok`.
...
### Task 4: Scenario/persona fold mapping + `(scenario, criterion)` cursor → `cursor.json`

Extend the fold to compute the resumable **cursor** — current phase, current `(scenario, criterion)`, and per-tuple done/pending counts — with the `__shared__` synthetic scenario and ADR-0012 `(criterion_id, persona="")` back-compat, and write it to a **separate `cursor.json`** artifact. **Do NOT machine-write `run-manifest.json` or `bug-log.json`** — those stay agent-authored; a fold overwrite would clobber their richer hand-filled fields (grill Q2/Q3). Because emission of `criterion_started`/`phase_entered` is deferred to Plan B (grill Q1), in a **live** Plan-A run the journal has only `criterion_verdict` events, so `cursor` reads `null` and `criteria_total == criteria_done` — the cursor logic here is real and **fixture-tested**, fed for real in Plan B. Relabel accordingly: this task ships the *cursor projection*, consumed by resume in Plan B.

**Files:**
- Modify: `skills/checkpointing-qa-memory/scripts/fold.jq` and `fold.py` — add a `cursor` block to the engine output: `{phase, criteria_total, criteria_done, personas:[…], scenarios:[…], cursor:{scenarioId,criterionId}|null}` where `cursor` = the first `(scenario, criterion)` that has a `criterion_started`/`plan_frozen` entry but **no** `criterion_verdict` (else `null` = run complete). `phase` = last `phase_entered` not yet `phase_exited`.
- Modify: `skills/checkpointing-qa-memory/scripts/fold.sh` — also `atomic_write` `cursor.json` from the engine's `cursor` block. (It continues to write `checkpoint.json` + `fold-anomalies.json`; it does **not** touch `run-manifest.json`/`bug-log.json`.)
...
- Produces: `cursor.json` = `{run_id, phase, criteria_total, criteria_done, personas, scenarios, cursor}`; `cursor` is the Plan-B resume entry point (`(scenarioId, criterionId)` or `null`). Shared criteria (`persona:""`) appear under scenario `__shared__`. **`run-manifest.json` and `bug-log.json` are untouched** by the fold.

- [ ] **Step 1: Create `tests/fold/fixtures/cursor.ndjson`**

A run with: `phase_entered verify`; two personas (`admin`, `user`) each starting a criterion, `admin`'s `C1` gets a verdict, `user`'s `C2` does **not**; a `__shared__` `C0` started + verdict. Expected cursor = `{scenarioId:"user",criterionId:"C2"}` (first started-without-verdict in seq order). Include exact lines. (This fixture is what exercises the cursor in Plan A, since live runs don't yet emit starts.)

- [ ] **Step 2: Write failing cursor tests**

Add to `tests/fold/run.sh`: fold `cursor.ndjson` → `cursor.json` has `.phase=="verify"`, `.cursor.scenarioId=="user"`, `.cursor.criterionId=="C2"`, `.criteria_done==2` (C0 + C1), `.scenarios | index("__shared__") != null`, `.personas | sort == ["admin","user"]`. Also assert `run-manifest.json`/`bug-log.json` are **not** created by the fold in this run dir (`[[ ! -e run-manifest.json ]]`). Also: a journal whose only records are legacy `persona:""` verdicts folds them under scenario `__shared__` (`.scenarios==["__shared__"]`) and yields `cursor:null` (no start events → complete). Run → FAIL (`cursor.json` not emitted yet).

- [ ] **Step 3: Extend `fold.jq` + `fold.py` with the cursor block**

Add the cursor computation to both engines (phase from `phase_entered`/`phase_exited` pairing; done = count of tuples with a verdict; total = distinct started/frozen/verdicted tuples; personas/scenarios distinct + sorted; cursor = first started-no-verdict by seq, else `null`). Keep the two engines semantically equal (extend the `dual-equiv` test to cover the cursor block).

- [ ] **Step 4: Emit `cursor.json` from `fold.sh`**

`atomic_write` the cursor block to `cursor.json` alongside `checkpoint.json`. Run: `bash tests/fold/run.sh` → all `ok`, `FAIL=0`; extend `dual-equiv` to canonically compare `cursor.json` across engines.
...
git add skills/checkpointing-qa-memory/scripts/fold.jq skills/checkpointing-qa-memory/scripts/fold.py \
        skills/checkpointing-qa-memory/scripts/fold.sh tests/fold/
git commit -m "feat(checkpointing): fold computes (scenario,criterion) cursor.json (__shared__, ADR-0012 back-compat); run-manifest/bug-log left agent-owned"
```
...
**Interfaces:**
- Produces:
  - `journal.sh append --child <name> <run> <event>` — appends to `.qa/runs/<run>/journal.<name>.ndjson` with a per-child seq, and **stamps `childId:"<name>"` + `childSeq:<n>` onto the event** (grill Q6/Q8) so the merge can dedup and the fold can detect a cross-child duplicate. (The event still gets its own `t` at append; the global `seq` is assigned at merge.)
  - `journal-merge.sh <run>` — under `flock .qa/runs/<run>/.journal.lock`, reads every `journal.<*>.ndjson`, orders their events (by child name then `childSeq` — a total, deterministic order), and **idempotently** appends them to `journal.ndjson`: any event whose `(childId, childSeq)` is **already present** in the main journal is skipped (grill Q6), so a re-run after a mid-merge crash is a no-op. It **re-stamps a monotonic global `seq`** continuing from the main journal's current max for the events it actually appends, then removes a child file **only after** its events are confirmed present in the main journal (append-then-verify-then-rm, so a crash before `rm` just re-dedups on the next run). Degrade: if `flock` is absent (`command -v flock` fails), fall back to a `mkdir`-based lock (`.journal.lock.d`) and note it.
  - fold: a discontinuous global `seq` in the main journal → `{"rule":"seq-gap","after":N}` anomaly; a single `(scenario,criterion,persona)` tuple carrying `criterion_verdict`s from **two different `childId`s** → `{"rule":"cross-child-duplicate","tuple":"…"}` anomaly (grill Q8). Neither aborts the fold.

- [ ] **Step 1: Write failing tests**

`tests/journal-merge/run.sh`: two children each append two events (`--child a`, `--child b`) → two `journal.a.ndjson`/`journal.b.ndjson` (each event carries `childId`/`childSeq`), main journal absent; `journal-merge.sh <run>` → main `journal.ndjson` has 4 lines with **contiguous** global seq `1..4`, child files removed; folding the merged journal yields both children's verdicts with **no** `seq-gap`/`cross-child-duplicate` anomaly. **Idempotency case (grill Q6):** run `journal-merge.sh` a second time with the same children re-created (simulating a crash that left them) → **no** duplicate lines added (dedup on `(childId,childSeq)`), seq still `1..4`. **Cross-child case (grill Q8):** two children both emit a `criterion_verdict` for the same `(scenario,criterion,persona)` → merge + fold → `cross-child-duplicate` anomaly present. Then a hand-crafted main journal with seqs `1,2,4` folds with a `seq-gap` anomaly. Run → FAIL.

- [ ] **Step 2: Implement `--child` in `journal.sh`**

Add the `--child <name>` option to `journal_append` (target `journal.<name>.ndjson` + per-child `next_seq` → `childSeq`; stamp `childId`/`childSeq` on the event). Keep the default (no `--child`) behaviour byte-identical (Task 1 tests still green — default events carry neither `childId` nor `childSeq`).

- [ ] **Step 3: Implement `journal-merge.sh`**

`flock` (or `mkdir`-lock degrade) around: collect `journal.*.ndjson` (sorted by name), build the set of `(childId,childSeq)` already in the main journal, append only the **new** child events (re-stamping global seq from the main journal's max), verify they landed, then `rm` each fully-merged child. Deterministic child order (lexicographic name). Time is **not** re-stamped (preserve each event's original `t`).

- [ ] **Step 4: Add `seq-gap` + `cross-child-duplicate` detection to both fold engines**

In `fold.jq`/`fold.py`: after collecting valid events, if the ascending distinct global `seq` list has a hole, push `{"rule":"seq-gap","after":<lastContiguous>}`; and if any `(scenarioId,criterionId,personaId)` group has `criterion_verdict`s bearing two distinct `childId` values, push `{"rule":"cross-child-duplicate","tuple":"<scenario>/<criterion>/<persona>"}`. Extend `dual-equiv`.
...
git commit -m "feat(checkpointing): idempotent fan-out sub-journal merge (childId dedup) under flock; fold seq-gap + cross-child-duplicate detection"
```
...
**Files:**
- Modify: `skills/checkpointing-qa-memory/SKILL.md` — add `journal.ndjson` + `fold-anomalies.json` + `cursor.json` to the Run Directory Layout tree; add `journal.sh`/`fold.sh`/`mutation-flag.sh`/`journal-merge.sh` to the Scripts Reference with one-line each; add a sentence to the ADR-0002 boundary section that "position = `fold(journal)`, never agent memory"; a mini-eval for torn-tail resume-position; **and one sentence that the fold owns `checkpoint.json`/`cursor.json`/`fold-anomalies.json` while `run-manifest.json`/`bug-log.json` remain agent-authored** (so nobody wires a fold overwrite of them later by accident — grill Q2/Q3). Keep body < 500 lines (currently 230).
- Modify: `CONTEXT.md` — confirm `journal`/`fold`/`scenario` entries match the implemented behaviour; add `fold-anomalies` and **`cursor`** (the `cursor.json` artifact / fold-exposed `(scenario,criterion)` position, distinct from the agent-authored `run-manifest`) if not present (one line each).

**Interfaces:** none (prose).

- [ ] **Step 1: Update SKILL.md**

Add the three files (`journal.ndjson`, `fold-anomalies.json`, `cursor.json`) to the layout tree, the four scripts to the Scripts Reference (each: name — one-line purpose), the position-is-fold sentence, the fold-owns-vs-agent-owns sentence, and mini-eval #6 (torn journal tail → fold still lands on the right `(scenario, criterion)` cursor, no half-written derived file). Confirm `awk 'END{print NR}'` < 500.

- [ ] **Step 2: Update CONTEXT.md**

Ensure `journal`, `fold`, `scenario`, `fold-anomalies`, `cursor` glossary lines are present and accurate to the implementation (add/adjust minimally; do not restate implementation detail — CONTEXT is a glossary).
...
**2. Grill-fix / exploration-gap alignment (Rounds 1–2, all folded in):**
...
- **Q1/Q9 cursor+act emission deferred** → Task 4 computes the cursor and Task 2 the act rules, but *emission* of `criterion_started`/`phase_entered`/`act_*` is Plan B; header + Task 4 relabel it "computed-and-fixture-tested, fed for real in Plan B" (no dead wiring, honest). ✅
- **Q2/Q3 artifact collision** → fold writes a separate `cursor.json`; it does **not** machine-write `run-manifest.json`/`bug-log.json` (those stay agent-authored). `bug_logged` events are journaled but no bug-log file is folded. Removes the drift-reconciliation scope entirely. ✅
- **Q4 perf** → full re-fold per checkpoint accepted (YAGNI, *n* bounded); recorded as a constraint + task comment. ✅
- **Q5 append atomicity** → 4 KB `PIPE_BUF` caveat stated in Task 1; torn-tail fold recovers the in-flight event (caller retries). ✅
- **Q6 idempotent merge** → child events carry `childId`/`childSeq`; merge dedups + append-then-verify-then-rm, so a mid-merge crash re-run is a no-op (Task 6). ✅
- **Q7 resume-order parity** → fold orders by first-seen position, last-wins value; Task 3 adds the explicit re-upsert characterization case. ✅
- **Q8 cross-child duplicate** → fold emits a `cross-child-duplicate` anomaly if one tuple has verdicts from two `childId`s (Task 6). ✅
...
- **Domain-modeling:** Task 7 adds `cursor` (the `cursor.json` artifact) to CONTEXT.md, distinct from the agent-authored `run-manifest`. ✅
...
**4. Type consistency:** event keys (`seq,t,event,scenarioId,criterionId,personaId,verdict,confidence,layer,evidenceRefs,kinds,bugRef,lastAction,nonUiActionReason`) are identical across Task 1's schema, Task 2's reducer mapping, Task 3's `checkpoint_verdict` assembly, and every fixture. The **derived** `checkpoint.json` record keys (`criterion_id,verdict,confidence,phase,last_action,evidence_refs,bug_ref,kinds,persona,nonUiActionReason,checkpointed_at`) match `checkpoint.sh`'s current output exactly (the characterization suite enforces it). `scenarioId==personaId` / `""→__shared__` / `__shared__→persona:""` is applied identically in Tasks 2, 3, 4. Fan-out child events additionally carry `childId`/`childSeq` (Task 6 only; default non-child events carry neither) — used identically by `journal-merge.sh`'s dedup and the fold's `cross-child-duplicate` check. `cursor.json` keys (`run_id,phase,criteria_total,criteria_done,personas,scenarios,cursor`) are identical across Task 4's engine block, `fold.sh` write, and the test assertions. `has_jq`/`has_py`/`die`/`atomic_write`/`canonical`/`journal_append`/`fold.sh`/`mutation-flag.sh` signatures are used identically everywhere they appear.
