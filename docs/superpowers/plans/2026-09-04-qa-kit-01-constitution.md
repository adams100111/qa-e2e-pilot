...
- **`constitution.md` is human-reviewable** — invariants (prose) + a roles summary table + the version/hash + a timestamp. The version/hash is also machine-readable (a fenced block or a sibling `.qa/constitution.state.json`) so re-runs can diff and specs can stamp.
...
**Files:**
- Create: `skills/checkpointing-qa-memory/scripts/constitution.sh` *(confirm sibling location by `ls skills/checkpointing-qa-memory/scripts/`; if the durable-state helpers live there, place it there; else `scripts/`)*
...
SH="$DIR/../../skills/checkpointing-qa-memory/scripts/constitution.sh"   # adjust to chosen location
pass=0; fail=0
...
run_engine() {
  ...
  check "$E render has state block" "$(printf '%s' "$out" | grep -c '"version": "abc123"')" "1"
  rm -rf "$T"
}
...
- [ ] **Step 3: Implement `constitution.sh`** — header + `has_jq`/`has_py`/`die`/`QA_ENGINE` idiom copied from a sibling (e.g. `ux-conventions.sh`/`journal.sh`). Three subcommands:
  ...
  - `render`: read the template, build the roles Markdown table from personas (`| id | role | plane |`), substitute `{{ROLES_TABLE}}`/`{{VERSION}}`/`{{TIMESTAMP}}`, append a fenced ```json `{roles:[{id,role,plane}], version}` state block. Deterministic given args.

- [ ] **Step 4: Run tests to pass** — `bash tests/constitution/run.sh` → `FAIL=0`; `bash -n` the script.

- [ ] **Step 5: Commit**
```bash
git add skills/checkpointing-qa-memory/scripts/constitution.sh tests/constitution/run.sh
