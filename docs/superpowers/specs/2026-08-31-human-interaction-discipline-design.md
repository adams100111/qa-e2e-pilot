...
Suggested task order for writing-plans: (1) ADR-0015; (2) `frontier` module + tests (2F); (3) action-trace + `human-action` gate in `checkpoint.sh` + tests; (4) `generating-qa-checklist` derives `human-action`; (5) `interaction-discipline.md` + `driving-browser-qa` rules + reconciliations (2C); (6) opt-out + config (2E); (7) orchestrator per-run selection wiring (2D) consuming the frontier module; (8) fixture cases + re-measure (§5); (9) review pass over the round-engine + confirming-discovered-roles.
