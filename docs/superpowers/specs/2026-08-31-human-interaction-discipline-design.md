...
Session setup: the driver launches Playwright MCP with `--save-session` (see §2E config). `session.md` is written to the MCP output dir (default `./.playwright-mcp/session.md`); the driver runs `parse-session-log.js` on it to produce the classified `sessionCalls` recorded into the criterion's `action-trace.json` (and copies `session.md` itself into the run dir for reviewer audit).
