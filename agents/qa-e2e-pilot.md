...
3. **Verify** — run criteria **sequentially on one driver by default** (ADR-0003). For each criterion, drive its **steps** and roll them into **one verdict**:
   ...
   - **Before checkpointing a `pass`**, write each evidence artifact the criterion's checklist `Kinds` field calls for, via **checkpointing-qa-memory**'s `record-evidence.sh <run-id> <criterion-id> <bake|computed|probe> ...` — one call per kind, capturing its printed `evidence/<crit>/<artifact>.json` path. Then checkpoint with `checkpoint.sh <run-id> <criterion-id> pass --kinds <derived-csv> --evidence-refs <paths>` (phase 5). The gate **may reject the pass** (non-zero exit) if an artifact is missing/empty/invalid — when it does, either go back and supply the missing evidence, or record the honest verdict `blocked` instead. **Never drop `--kinds` to force a pass through** — that defeats the gate this pipeline exists to enforce. Non-`pass` verdicts (`fail`/`blocked`/`deferred`/`error`) checkpoint normally, ungated.
