...
# ---------------------------------------------------------------------------
# apply: rejects a key that is NOT currently an open act (never intended,
# or already committed) — nonzero, nothing journaled.
# ---------------------------------------------------------------------------
( cd "$WORK" && bash "$RECON" apply qa4 "qa4:admin:AC1" --readbacks "$RB_ALL" >/dev/null 2>&1 ); rc_not_open=$?
check "apply: key not open -> rejected (nonzero)" "$([[ $rc_not_open -ne 0 ]] && echo nonzero || echo zero)" "nonzero"
