...
# reconcile).
# ---------------------------------------------------------------------------
( cd "$WORK" && bash "$EMIT" started qp2 admin AC1 admin >/dev/null )
...
# ---------------------------------------------------------------------------
( cd "$WORK" && bash "$EMIT" started qp3 admin AC1 admin >/dev/null )
( cd "$WORK" && bash "$EMIT" act-intent qp3 admin AC1 admin --criterion "$MUT_CRIT" --write-set "$WS_ALL" >/dev/null )
( cd "$WORK" && bash "$EMIT" started qp3 user AC2 user >/dev/null )
...
# ---------------------------------------------------------------------------
( cd "$WORK" && bash "$EMIT" started qa1 admin AC1 admin >/dev/null )
...
# ---------------------------------------------------------------------------
( cd "$WORK" && bash "$EMIT" started qa2 admin AC2 admin >/dev/null )
...
# ---------------------------------------------------------------------------
( cd "$WORK" && bash "$EMIT" started qa3 admin AC3 admin >/dev/null )
...
# ---------------------------------------------------------------------------
( cd "$WORK" && bash "$EMIT" started qa6 admin AC1 admin >/dev/null )
...
# excluded from.
# ---------------------------------------------------------------------------
( cd "$WORK" && bash "$EMIT" started qa7 admin AC1 admin >/dev/null )
...
# ---------------------------------------------------------------------------
( cd "$WORK" && bash "$EMIT" started qa8 admin AC1 admin >/dev/null )
...
# journaled.
# ---------------------------------------------------------------------------
( cd "$WORK" && bash "$EMIT" started qa5 admin AC1 admin >/dev/null )
...
if command -v jq >/dev/null 2>&1 && command -v python3 >/dev/null 2>&1; then
  ...
  run_plan_scenario() {
    ...
    ( cd "$dir" \
        ...
        && PATH="${run_path:-$PATH}" "${run_bash[@]}" "$EMIT" started dual-plan user AC2 user >/dev/null \
  ...
  run_apply_done_scenario() {
    ...
    if [[ "${1:-}" == "--py" ]]; then run_bash=("$BASH_BIN"); run_path="$FAKEBIN"; shift; fi
    ( cd "$dir" \
        && PATH="${run_path:-$PATH}" "${run_bash[@]}" "$EMIT" started dual-done admin AC1 admin >/dev/null \
  ...
  run_apply_escalate_scenario() {
    ...
    if [[ "${1:-}" == "--py" ]]; then run_bash=("$BASH_BIN"); run_path="$FAKEBIN"; shift; fi
    ( cd "$dir" \
        && PATH="${run_path:-$PATH}" "${run_bash[@]}" "$EMIT" started dual-esc admin AC1 admin >/dev/null \
  ...
  run_apply_corroborate_scenario() {
    ...
    if [[ "${1:-}" == "--py" ]]; then run_bash=("$BASH_BIN"); run_path="$FAKEBIN"; shift; fi
    ( cd "$dir" \
        && PATH="${run_path:-$PATH}" "${run_bash[@]}" "$EMIT" started dual-corr admin AC1 admin >/dev/null \
