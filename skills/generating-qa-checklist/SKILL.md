...
- [ ] For **empty/0-value** and **negative/out-of-range**, the verdict hinges on TWO baked checks, NOT on how the UI *looks* (a "rejected-looking" screen is not evidence — this is the exact gap a blind re-measurement caught, missing J4/F4). Emit these as `Kinds: bake` and assert BOTH: **(a)** submit the invalid value through the real UI, then BAKE the backend read-back — the invalid value must be **ABSENT** (a 0-share add persists nothing; a negative/out-of-range value is not stored); a value that IS present in the read-back is the **F4** bug. **(b)** capture the success indicator/toast text in the same step — a green "success"/"added"/"saved" toast that fires while the read-back shows nothing persisted is the **J4** bug (a dishonest rejection). A `pass` requires the read-back to confirm non-persistence AND no success toast; never mark pass because the input "looked rejected" or the row didn't visibly appear.
- [ ] For **named/boundary/reserved-looking value**, pick an input drawn from the domain: a value colliding with a UI label/placeholder, a reserved word, a boundary-length string, or a value equal to an adjacent field — anything that risks silent normalization or a swallowed write. Because a name-triggered silent drop (the J1 class) is nearly invisible without baking, ALWAYS bake the read-back and confirm the exact named row is present — a success toast is never sufficient.
...
- `C-FOUNDERS-EDGE-01` (0-share input) — create a founder with `shares = 0`. `Kind: business-rule`, `Tags: probe-needed` → the action mutates (`create` is a mutating verb, so `mutation-flag.sh` derives `human-action`) and `business-rule` maps to `computed`, not `bake` → `Kinds: computed,human-action,probe`. Oracle: a 0-share founder must be rejected or persist visibly at 0% — a "Success" toast with no row created is a `fail` (catches J4).
- `C-FOUNDERS-EDGE-02` (negative-share input) — create a founder with `shares = -500` entered via `browser_type`/`browser_fill_form` (human-path tools, ADR-0015), then confirmed with `scripts/react-set-input.js`'s read-back per driving-browser-qa's numeric read-back rule (the script is read-only — it never enters a value) so a swallowed keystroke can't masquerade as a rejection. `Kind: business-rule`, `Tags: probe-needed` → same derivation as EDGE-01 (`create` mutates, `business-rule` maps to `computed`) → `Kinds: computed,human-action,probe`. Oracle: negative shares must be rejected server-side, not silently coerced into a valid 0-share row (catches F4).
- `C-FOUNDERS-EDGE-03` (every-3rd add) — add founders 1, 2, 3 in sequence (N=3 from the Step 2 multiplicity schedule). `Kind: multiplicity-N` → `multiplicity-N` is a bake-kind (adds `bake`) and `add` is a mutating verb (adds `human-action`) → `Kinds: bake,human-action`. Oracle: the read-back count after founder #3 is exactly 3, not 2 (catches J3).
- `C-FOUNDERS-EDGE-04` (named/boundary value) — create a founder named literally "Founder" (collides with the form's own placeholder). `Kind: happy-path`, `Tags: probe-needed` → `happy-path` is a bake-kind (adds `bake`), `create` mutates (adds `human-action`), `probe-needed` adds `probe` → `Kinds: bake,human-action,probe`. Oracle: the founder is present in the read-back list, not just in the success toast (catches J1).
- `C-FOUNDERS-EDGE-05` (delete-then-reconcile) — delete one of the 3 founders. `Kind: downstream-cascade` → `downstream-cascade` is a bake-kind (adds `bake`) and `delete` is a mutating verb (adds `human-action`) → `Kinds: bake,human-action`. Oracle: the founders count badge and ownership-percentage sum re-reconcile to 2 founders summing to 100%, not a stale total left over from 3 (catches F3).
...
Each row's `Kind`/`Tags` came straight from the Step 3 table; `Kinds`/`probeNeeded` were derived by Step 7, not hand-set — and Step 7's own derivation follows `required-kinds.sh` exactly: `business-rule` yields `computed` (never `bake` on its own), and a mutating action verb in the criterion's action text (`create`, `add`, `delete`, …) independently adds `human-action` regardless of `Kind`. None of these five bugs required a new verification technique — only a criterion that existed to point verification at them.
...
With this skill: Step 3's terminal/locked-state row emits `C-FINALIZE-EDGE-07` — click Finalize again while `roundStatus=COMPLETE`. `Kind: business-rule`, `Tags: probe-needed` → `business-rule` is not one of `required-kinds.sh`'s bake-kind enum, so it maps to `computed` instead (not `bake`); neither `click` nor `Finalize` matches `mutation-flag.sh`'s mutating-verb list, so no `human-action` is added either; `probe-needed` adds `probe` → `Kinds: computed,probe`. Oracle: the repeat is an idempotent no-op (roundStatus stays COMPLETE, holdings unchanged) OR an explicit rejection surfaced to the user — it must NEVER be a silent "success" that then blocks all future holdings creation with no recovery path. The baking step re-reads holdings-creation availability after the repeat click; finding it permanently disabled with no error shown is a `fail`, suspected layer `FE` (matches BUG-004 exactly). Caught only because the row forces a second finalize invocation from the terminal state — the same click sequence a real user eventually performs by mis-click or double-submit.
