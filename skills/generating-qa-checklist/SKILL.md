...
- `C-FOUNDERS-EDGE-02` (negative-share input) — create a founder with `shares = -500` entered via `browser_type`/`browser_fill_form` (human-path tools, ADR-0015), then confirmed with `scripts/react-set-input.js`'s read-back per driving-browser-qa's numeric read-back rule (the script is read-only — it never enters a value) so a swallowed keystroke can't masquerade as a rejection. `Kind: business-rule`, `Tags: probe-needed` → `Kinds: bake,probe`. Oracle: negative shares must be rejected server-side, not silently coerced into a valid 0-share row (catches F4).
