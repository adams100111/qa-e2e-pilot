...
**Expected `fold-anomalies.json` entries (not errors):** the orchestrator now emits `plan_frozen`/`plan_amended` (Generate→Verify boundary), `criterion_started` (per-criterion, at Verify's start), and `act_intent`/`act_committed` (bracketing a mutating criterion's act) via `journal-emit.sh` — see the Scripts Reference below — so a clean run normally shows NO `verdict-without-started` anomalies. If a caller writes a verdict via `checkpoint.sh` without ever having called `journal-emit.sh started` for that tuple (e.g. a script or fixture driving `checkpoint.sh` directly, bypassing the orchestrator prose), the fold still records the verdict and flags it with a benign `verdict-without-started` anomaly rather than rejecting it — the "record-the-verdict + flag" rule, never a suppressed verdict. Genuine problems are the other rules (`unparseable-line`, `seq-gap`, `cross-child-duplicate`, `fsync-unavailable`, `act-committed-no-intent`). **Two of them are now load-bearing outside the fold:** `unparseable-line` and `seq-gap` mean the run's own *record* is damaged, so `report-to-junit.sh` marks such a run run-level `UNVERIFIED` and fails the build (see [running-in-ci.md](../../docs/running-in-ci.md)). Every other rule — including the findings-ledger pair `finding-url-oversize` and `finding-detail-missing` — is surfaced as a **count only** on the report's `qa.foldAnomalies` property and never reddens a run. **What is new is only the visibility**, not the anomaly: `fold-anomalies.json` has always recorded these, and `report-to-junit.sh` now prints their counts. Note the tuple that decides `verdict-without-started`: `checkpoint.sh` derives the verdict's `scenarioId` as the persona id (or `__shared__` when no persona is given), which is the same convention `journal-emit.sh started` and the freeze use, so an orchestrator-driven run's `criterion_started` matches its verdict and the anomaly stays at zero. The fold also decides it from the started flag **as of the kept verdict**, so a `criterion_started` arriving *after* that verdict does not retroactively erase it.

### Journal event types — FOURTEEN, not twelve

`journal.sh`'s EVENT SCHEMA comment block is the contract; read it there. Two events were added by the error-honesty work and are the ones to know about here:

- **`finding_observed`** `{criterionId, source, channel, method, url, urlLen?, status, originClass, statusClass, message, detailRef}` — one observed error finding. The fold reduces these as a **keyed set** (never last-wins) over a `findingKey` it **derives** as `<criterionId>|<source>|<method>|<url>|<status>` — a `console` source keys on the normalized, capped `message` instead of the url. A caller-supplied `findingKey` is advisory and never trusted: dedup must be a property of the fold, not of whichever emitter ran, or a resumed run re-observes and double-counts. `message` is capped at ~200 chars and `url` at 1024, with the full url in the `detailRef` detail file under `evidence/<criterion>/findings/` and its full length in `urlLen` — the PIPE_BUF boundary in `journal.sh` is why. `checkpoint.json` carries the resulting set as a `findings` array in first-seen (ascending-`seq`) order.
- **`capture_probed`** `{channel}` — the **once-per-run capture-channel canary**, appended by `checkpoint.sh`'s upsert behind a scan-for-an-existing-event guard. It is deliberately NOT in the agent's Phase 0 pre-flight: `/qa-resume` dispatches a resumed run straight into its recorded phase, so a Phase-0 assertion is silently skipped on every resume, and the journal is the only place a once-per-run fact survives one. `channel` is `toolstream` (a non-empty `toolstream.jsonl` exists) | `driver-log` (a resolvable `--save-session` log) | `none`. **`none` is an honest degrade, never an error here** — but `report-to-junit.sh` treats anything that is not `toolstream`/`driver-log`, *including the event being absent entirely*, as run-level `UNVERIFIED`.

**Registering a new event type is two edits, both mandatory.** `journal.sh append` accepts any non-empty `event` string, so an unregistered event appends fine and is then silently discarded by the fold as an `unknown-event` anomaly. Add the name to **both** `fold.sh`'s `KNOWN_EVENTS_JSON` literal **and** the duplicated python3 set inside its `parse_journal_py`.
...
| `scripts/journal.sh append <run-id> <event-json>` | Append one validated, seq-stamped event to `journal.ndjson` — the append-only source of truth. Its EVENT SCHEMA comment block is the contract for all **14** event types (incl. `finding_observed`, `capture_probed`); it accepts any non-empty `event` string, so a type not registered in **both** of `fold.sh`'s engines is silently discarded by the fold |
